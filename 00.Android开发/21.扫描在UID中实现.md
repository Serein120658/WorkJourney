...
export default {
	...
	data() {
		return {
			...
			scanInputValue: '',       // 扫描输入框的值（隐藏）
			autoFocus: false,         // 自动聚焦（初始为false，避免弹出键盘）
			...
			// 扫描输入监听
			scanInputTimer: null,     // 扫描输入计时器
			
			// 页面状态
			isPageActive: false,      // 页面是否激活
			
			// 自动重新聚焦
			refocusTimer: null        // 重新聚焦计时器
		}
	},
	...
	methods: {
		...
		registerScanKeyListener() {
			...
			try {
				...
				console.log('✓ 手柄按键监听已注册');
			} catch (e) {
				console.error('✗ 注册手柄按键监听失败:', e);
			}
			// #endif
		},
		...
		unregisterScanKeyListener() {
			...
			try {
				if (this.main && this.receiver) {
					...
					this.receiver = null;
					this.filter = null;
					console.log('✓ 手柄按键监听已注销');
				}
			} catch (e) {
				console.error('✗ 注销手柄按键监听异常:', e);
			}
			// #endif
		},
		 ...
		 */
		onScanInput(e) {
			console.log('onScanInput 触发, 输入值:', e.detail.value);
		 ...
		 */
		onScanConfirm(e) {
			console.log('onScanConfirm 触发, 输入值:', e.detail.value);
			const value = e.detail.value.trim();
			if (value) {
				this.handleScanData(value);
			}
