...
通过在控制台请求页面所请求的入口如下：在  EnterController.java  文件中  

发现有三种请求入口   页面点击请求的  http://localhost:9999/  下面的内容
...
1. Enter   
   ...
   ```java
    @RequestMapping(value = "/Enter", method = {RequestMethod.GET, RequestMethod.POST})
       public AjaxResult enterGet(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
           String rpc = request.getParameter("rpc");
           ReqData req = new ReqData(tokenService.getTokenID(request), ServletUtils.getParamMap(request), SecurityUtils.getLogUserMap());
           JSONObject jsonRe;
           Object res;
           if (StringUtil.notNull(rpc)) {
               res = enterRpc.getMsgData(request);
               //跳转外部请求
           } else {
           ...
           if (res instanceof String) {
               jsonRe = new JSONObject(String.valueOf(res));
           } else {
               jsonRe = (JSONObject) res;
           }
   
           if (jsonRe.has("error")) {
               return AjaxResult.error(jsonRe.optString("error"));
           } else {
               return AjaxResult.success("操作成功", jsonRe.has("re") ? jsonRe.get("re").toString() : jsonRe.toString());
           }
       }
   ```
...
2. LayEnter
   ...
   ```java
   @RequestMapping(value = "/LayEnter", method = {RequestMethod.GET, RequestMethod.POST})
   public String laytableGet(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
       String rpc = request.getParameter("rpc");
       ReqData req = new ReqData(tokenService.getTokenID(request), ServletUtils.getParamMap(request), SecurityUtils.getLogUserMap());
       Object res;
       if (StringUtil.notNull(rpc)) {
...
3. EnterRpc
   ...
   *  rpc: 系统标识 如tongbang  fuyou  fyyypt等，注意在sys.properties中要有对应系统地址(标识+Address) 如tongbangAddress 
   ...
   // js文件中
   $.sm(function(re, err){    if(re){    } }, ["其他系统消息"], {rpc: 'fuyou'});
   ```

   接口代码：

   ```java
   @RequestMapping(value = "/EnterRpc", method = {RequestMethod.GET, RequestMethod.POST})
   public String enterRpcPost(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
       ...
       String strre = (String) enterRpc.getMsgData(request);
