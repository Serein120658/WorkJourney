...
当我按照上面的例子去实现批量查找但是携带参数传递到后端，会出现意想不到的错误
...
$("#btnBatchWithCondition").click(function () {
    ...
    //     {
    //         menu_name: "msgUtils和batch批量消息处理",
    //         id: 227
    //     }
    // ),$.msgwhere()])
    console.log(arrsm)
...
<font color=red>[ERROR] [http-nio-9999-exec-2] - com.whoami.db.MsgKit.checkArrParam(MsgKit.java:278) - -------where消息，检测到JSON格式有误. [msgid='gx.selectjsonwhere', arrpm='1', value='{"keyword":["msgUtils"]}']</font>

![image-20250801161314382](10.09问题解决及批量消息发送练习.assets/image-20250801161314382.png)

查看源码：分析原因：

第一个元素 "gx.selectjsonwh..." 和第二个元素 "{%22keyword%22:..." 都不包含 "msg_where"。
只有第三个元素 "{%22msg_where%2..." 包含 "msg_where"。
问题原因：在循环中，当 i = 0 或 i = 1 时，arrpm[i] 不包含 "msg_where"，因此会触发错误处理逻辑（记录日志并返回错误信息）。



这样看来，可能是前端传递的问题   但是除此之外好像也不知道如何传递  

不可能将源码的  ++i   改为 i++ ?



可能的原因是一般情况下不会批量更新或者删除操作
