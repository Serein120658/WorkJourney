...
1. 通过前端传递过来的文件，先使用InputStream流去读取文件，获取原始文件名然后根据后缀走不同的方法
2. 如果是xlsx结尾的，new 一个 XSSFWorkbook  对象
3. 获取第一个sheet  再获取相应的表头
4. 分配处理行数据  去除空行和数据清洗
...
场景：前面我们实现了使用SAX事件驱动去解析excel表格的数据，但是到与数据库联动的时候犯难了，因为按照之前的经验，如果要与数据库交互，那么所采用的就是使用msgUtils.sm() 去模拟前端发送消息体，然后在xml文件里面找到相应的消息，再通过消息类型去与数据库交互的，但是数据量都不大，现在如果导入了excel并解析后，如果需要将解析的数据存入到数据库当中，那么该如何写呢？ 使用循环去一次一次调用消息吗？

![image-20250806092438018](11.Excel表格的导入与解析.assets/image-20250806092438018.png)

解析得到是相应的数据，objwhere依次赋值给每个？像下面这样？

objwhere.put("email",new String[]{va})
