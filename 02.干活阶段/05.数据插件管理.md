...
之前的页面太宽，感觉不太行，美化一下页面

![image-20250819114105956](05.数据插件管理.assets/image-20250819114105956.png)

![image-20250819114126236](05.数据插件管理.assets/image-20250819114126236.png)

### BUG修复

#### 1、添加页面点击保存未验证

之前我们在添加插件的时候需要对必填字段验证是否输入了相应的东西，但是现在点击了保存之后，就直接关闭模态框

对于验证，我们在代码中是通过手动验证，也就是说是没有使用原本表单自带的form.verfy()  方法去验证的  这就导致了上面的bug，因为之前的也是用相应的方法去验证字段是否输入的问题。

#### 2、点击编辑插件

保存后没有关闭模态框，因为编辑和添加使用的是同一个页面

#### 修复

现在验证了，但是有关不掉了// TODO 待解决
